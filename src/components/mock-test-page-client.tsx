...
export function MockTestPageClient({
    ...
    const selectTopic = (topic: string | null) => {
        ...
        if (topic) {
            router.replace(`/mock-tests?topic=${encodeURIComponent(topic)}`, { scroll: false });
        } else {
            router.replace('/mock-tests', { scroll: false });
        }
    };
                ...
                {!selectedTopic && !searchQuery && (
                    <>
                        <div className="mb-12">
                            ...
                            <p className="text-lg text-foreground/70 dark:text-slate-300/70">Select a topic to see available tests and enroll</p>
                        </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-16">
                            {Object.entries(groupedByTopic).map(([topic, tests]) => {
                                ...
                                const seriesName =
                                    ...
                                    tests[0]?.seriesTitle ||
                                    'Unknown';
                                const hasSeriesPricing = Boolean(tests[0]?.seriesPrice && tests[0].seriesPrice > 0);
                                const seriesPrice = tests[0]?.seriesPrice || 0;
                                ...
                                const savings = totalIndividualCost - seriesPrice;
                                ...
                                return (
                                    <button
                                        ...
                                        className="text-left block h-full w-full"
                                    >
                                        <Card
                        ...
                        {displayedTests.length > 0 ? (
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                                {displayedTests.map((test) => (
                                    ...
                                    >
                                        <Card
                                            className={`overflow-hidden hover:shadow-xl transition-all duration-300 flex flex-col cursor-pointer group bg-white dark:bg-slate-800/50 border border-white/80 dark:border-slate-700/50 h-full ${getDifficultyBgColor(test.difficultyLevel)}`}
                                        >
                                            ...
                                            <div className="border-t border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/30 px-6 py-4">
                                                <div className="space-y-3">
                                                    <div className="flex items-center justify-between">
                                                        ...
                                                        <span className="inline-flex items-center rounded-md bg-blue-600 text-white text-sm font-medium h-9 px-3 gap-1">
                                                            View & Enroll <ChevronRight className="w-4 h-4" />
                                                        </span>
                                                    </div>
