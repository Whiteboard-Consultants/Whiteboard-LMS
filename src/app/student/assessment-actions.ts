...
export async function getTestAttemptForResults(attemptId: string) {
  ...
  try {
    ...
    if (questionsError) {
      console.error('Test questions error:', questionsError);
    }

    // If no questions found, we'll still show the basic result without detailed review
    const questionsArray = questions || [];

    // Check if we have answer_details with question data stored from submission
    console.log('Answer details from attempt:', attempt.answer_details);
    
    let finalQuestionsArray = questionsArray;
    
    // If no questions from test_questions table, try to reconstruct from answer_details
    if (questionsArray.length === 0 && attempt.answer_details && Array.isArray(attempt.answer_details)) {
      console.log('Reconstructing questions from answer_details');
      finalQuestionsArray = attempt.answer_details.map((detail: any, index: number) => ({
    ...
    const transformedAttempt = {
      ...
      course_id: attempt.course_id, // Use actual course_id
      enrollment_id: attempt.enrollment_id,
      answers: attempt.answers || [],
      questions: finalQuestionsArray.map((q, index) => ({
        id: q.id,
        questionText: q.questionText || q.question_text,
        options: Array.isArray(q.options) ? q.options : (typeof q.options === 'string' ? JSON.parse(q.options) : []),
        correctAnswerIndex: typeof q.correctAnswerIndex !== 'undefined' ? q.correctAnswerIndex : (parseInt(q.correct_answer) || 0),
        explanation: q.explanation || ''
      })),
      score: attempt.score || 0, // Use score field from database
      total_questions: attempt.total_questions || finalQuestionsArray.length, // Use database value first, fallback to questions count
